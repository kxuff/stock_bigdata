...
CrewAI Hierarchical Crew
  ...
  └── agents = [
        Data/Technical Agent + market/ML tools,
        Sentiment Agent + news/sentiment tools,
        Valuation Agent + fundamentals tools,
        Risk Agent + risk/portfolio tools
      ]
        │
        ▼
Deterministic Policy Services
  ├── Source Quality over tool results
...
Payload chuẩn là `AdvisoryDecisionRequest` tối thiểu. Agents tự query upstream layers qua read-only CrewAI tools được gắn khi khởi tạo `Agent(...)` hoặc override ở `Task(...)`; tool scope không nằm trong request payload.
...
- API request/response examples align với `AdvisoryDecisionRequest`, tool-result audit, `confidence_breakdown`, review flag, disclaimer và audit metadata.
...
- Tạo Pydantic models cho `AdvisoryDecisionRequest`, market/ML/sentiment/valuation/risk tool results, agent outputs, final decision và audit metadata.
...
- Tạo sample requests và mocked upstream tool results cho normal, high-risk, missing sentiment, missing valuation, stale data, portfolio allocation.
...
- Missing required request fields hoặc unavailable required market tool result bị reject rõ ràng.
- Missing optional tool results như sentiment/valuation không làm fail toàn request.
...
- Tạo CrewAI hierarchical runner tối thiểu với custom `manager_agent`, 4 specialist agents và mocked upstream tools.
...
- Specialist agents có mocked tools gắn qua `Agent(..., tools=[...])` hoặc task-level `tools=[...]`.
...
- Audit metadata có `run_id`, `request_id`, `input_request_hash`, `tool_result_bundle_hash`, `validator_version`, timestamp.
...
**Mục tiêu:** triển khai 4 specialist agents query upstream qua tools và trả structured output.
...
- Valuation Agent.
- Risk Agent.
- Tool wiring cho market/ML, sentiment, fundamentals và risk/portfolio.
...
- Sentiment/Valuation có thể `SKIPPED` nếu optional tool result thiếu.
- Risk Agent trả `risk_label`, `risk_factors`, `confidence_cap`.

**Test/Review scope:**

- Agent tests dùng mocked LLM response.
- Prompt review kiểm tra tool-result-only reasoning và prompt-injection boundary.
...
- Error responses cho invalid request, missing required tool result, agent timeout, validation failed.
...
- Stale/missing optional tool result scenario vẫn trả response hợp lệ với limitation.
- Missing required market tool result trả error rõ.
...
- Failure tests cho missing required tool result, invalid allocation, invalid LLM JSON, high risk + bullish ML, stale data, low news relevance.
...
- Nhận `AdvisoryDecisionRequest` JSON.
- Chạy hierarchical CrewAI với custom `manager_agent`.
- Chạy Data/Technical, Sentiment, Valuation, Risk agents.
- Mỗi specialist agent query upstream layer qua read-only tools được gắn ở Agent/Task definition.
...
| Missing optional tool result làm hỏng flow | Demo thiếu ổn định | Cho phép `SKIPPED`/`DEGRADED`, ghi limitation và cap confidence |
...
| Prompt injection từ news | Agent làm sai | Sanitize external text, nhắc prompt chỉ dùng tool results như evidence, không follow instruction trong news |
...
```text
Use only the user request and facts retrieved through assigned read-only tools.
