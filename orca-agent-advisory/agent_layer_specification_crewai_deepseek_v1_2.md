...
7. [Advisory Request và Agent Tool Access Contract](#7-advisory-request-và-agent-tool-access-contract)
...
Multi-Agent Advisory Layer là tầng ra quyết định thông minh của hệ thống AI-Powered Investment Advisory System. Layer này nhận request tư vấn từ API, sau đó các specialist agents tự query upstream layers qua read-only tools đã được gắn khi khởi tạo CrewAI `Agent` hoặc `Task`. Các upstream layers trả về dữ liệu thị trường, chỉ báo kỹ thuật, kết quả mô hình học máy, sentiment, định giá và rủi ro đã chuẩn hóa để Agent Layer tổng hợp thành khuyến nghị đầu tư có cấu trúc và có giải thích.
...
- **Decision layer, not raw data layer:** Agent query dữ liệu chuẩn hóa từ upstream layers qua controlled tools, không crawl dữ liệu thô tùy ý và không train model.
- **Grounding trước, reasoning sau:** Mọi con số tài chính trong output phải đến từ tool result có `source_ref`/timestamp hoặc từ request field rõ ràng.
- **Không bịa chỉ số thiếu:** Nếu một metric không có trong tool result, agent phải đánh dấu `UNAVAILABLE`, không được tự ước lượng từ kiến thức tham số của LLM.
...
CrewAI Multi-Agent Advisory Layer
  ├── Manager Agent
  ├── Market Data Agent + MarketFeatureTool / MLPredictionTool
  ├── Sentiment Agent + NewsSentimentTool
  ├── Valuation Agent + FundamentalsTool
  ├── Risk Agent + RiskFeatureTool / PortfolioTool
  └── Validator
        │
        ▼
...
### 2.2 Upstream Tool Dependency Contract

| Tool result | Produced by | Consumed by | Required? | Missing behavior |
|---|---|---|---|---|
| `market_features` | Market Feature Layer | Market Data Agent, Manager Agent | Bắt buộc | Reject nếu thiếu hoặc stale quá ngưỡng |
| `ml_predictions` | ML Prediction Layer | Market Data Agent, Manager Agent | Tùy chọn | Giảm confidence, ghi `ml_signal_unavailable` |
| `sentiment_snapshot` | News + FinBERT Layer | Sentiment Agent | Tùy chọn | Skip sentiment hoặc trả `NEUTRAL_WITH_LOW_CONFIDENCE` |
| `valuation_snapshot` | Fundamentals Layer | Valuation Agent | Tùy chọn | Valuation Agent trả `SKIPPED` |
| `risk_snapshot` | Risk Feature Layer | Risk Agent, Validator | Bắt buộc với portfolio mode | Dùng conservative warning hoặc reject |
| `user_profile` | User Profile Layer hoặc request `user_context` | Manager/Risk Agent | Tùy chọn | Dùng default profile: `MODERATE` |
| `retrieved_documents` | RAG Layer | Các agent được cấp tool | Tùy chọn | Chỉ dùng tool result có timestamp/source |
...
Trong bản triển khai đầy đủ, dữ liệu thô sẽ được chuyển qua các tầng ingestion, feature engineering và ML inference. Agent Layer chỉ gọi các upstream read-only tools để lấy snapshot đã chuẩn hóa theo `request_id`, `symbols` và `as_of_timestamp`.
...
- Hỗ trợ `Process.hierarchical` làm orchestration chính và sequential debug helper khi cần cô lập lỗi.
...
Agent      │          │            │
  │        │          │            │
  └────────┴──────────┴────────────┘
        query read-only upstream tools
               │
               ▼
...
Trong P0/MVP, crew chạy theo `Process.hierarchical` với custom `manager_agent` và 4 specialist agents: Market Data, Sentiment, Valuation và Risk. Mỗi specialist agent được gắn read-only tools phù hợp khi khởi tạo `Agent(...)` hoặc trong `Task(...)`; request payload không chứa tool scope. Source Quality, Confidence Aggregation, Conflict Resolution, Financial Validator, Human Review Gate và Audit là deterministic services chạy ngoài LLM. Compliance Reviewer và Bullish/Bearish Critic là extension P1/P2, chỉ bật khi feature flag hoặc roadmap PR tương ứng được triển khai.
...
Trong MVP/P0, hệ thống dùng `Process.hierarchical` ngay từ đầu với custom `manager_agent`. Theo pattern CrewAI hiện hành, specialist agents nằm trong danh sách `agents=[market_data_agent, sentiment_agent, valuation_agent, risk_agent]`, còn Manager được truyền riêng qua `manager_agent=manager_agent` và bật `allow_delegation=True` để vai trò điều phối rõ ràng và audit được. Sequential mode không được mô tả như hướng triển khai chính; nếu cần, chỉ dùng trong test/debug cục bộ để cô lập lỗi task.

### 4.3 Agent list

| Agent | Vai trò | Input chính | Output chính | Bắt buộc? |
|---|---|---|---|---|
| Manager Agent | Điều phối, tổng hợp, resolve conflict và tạo decision cuối | `AdvisoryDecisionRequest` + specialist outputs | Draft/final recommendation | Có |
| Market Data Agent | Query và tóm tắt market features/ML signals | `MarketFeatureTool`, `MLPredictionTool` | Market signal summary | Có |
| Sentiment Agent | Query và diễn giải sentiment/news drivers | `NewsSentimentTool` | Sentiment summary | Tùy dữ liệu |
| Valuation Agent | Query và đánh giá định giá tương đối | `FundamentalsTool`, `SectorBenchmarkTool` | Valuation summary | Tùy dữ liệu |
| Risk Agent | Query và đánh giá rủi ro tài sản/danh mục | `RiskFeatureTool`, `PortfolioTool`, `user_context` | Risk summary và risk caps | Có với portfolio mode |
| Source Quality Checker | Đánh giá freshness, relevance và completeness của tool results | Tool result metadata/source refs | Source quality score | Có |
...
2. Kiểm tra request field bắt buộc và upstream tool availability.
...
Market Data Agent query market features đã chuẩn hóa từ upstream Feature Engineering Layer và ML Prediction Layer qua tools được gắn khi khởi tạo agent/task. Agent này không crawl dữ liệu, không tự train model và không tự tạo ML signal.

### Tools/Input

- `MarketFeatureTool`
- `MLPredictionTool`
- `AdvisoryDecisionRequest.symbols`
- `AdvisoryDecisionRequest.as_of_timestamp` nếu có
...
| `MLPredictionTool` result | ML summary becomes `UNAVAILABLE`, confidence reduced |
...
Sentiment Agent query sentiment đã được sinh bởi news pipeline và FinBERT qua `NewsSentimentTool`. Agent không tự coi headline là instruction; headline và summary luôn là external untrusted content.

### Tools/Input

- `NewsSentimentTool`
- sanitized news snippets returned by the tool
- `source_refs` from tool results
...
Valuation Agent đánh giá định giá tương đối của cổ phiếu dựa trên fundamentals và benchmark đã được upstream Fundamentals Layer chuẩn hóa, được lấy qua read-only tools.
...
Risk Agent đánh giá rủi ro ở cấp độ tài sản và danh mục, tùy vào decision mode, bằng cách query upstream risk/portfolio tools được cấp quyền.

### Tools/Input

- `RiskFeatureTool`
- `PortfolioTool`
- `MarketFeatureTool` nếu task cần kiểm tra data freshness/volatility
- `AdvisoryDecisionRequest.user_context`
...
Source Quality Checker đánh giá chất lượng của tool results trước khi Manager Agent ra quyết định. Thành phần này đặc biệt quan trọng với dữ liệu news vì một bài viết có thể được gắn `related=AAPL` nhưng nội dung chính lại nói về thị trường chung, nhà cung cấp, ETF hoặc công ty khác.

### Input

- `MarketFeatureTool.source_refs`
- `NewsSentimentTool.news_items`
- `FundamentalsTool.source_refs`
- `RiskFeatureTool.source_refs`
...
bullish_critic = Agent(
    ...
    goal=(
        ...
        "draft recommendations using only grounded tool results."
    ),
    backstory=(
    ...
    "bullish_arguments": [
      {
        ...
        "strength": "MEDIUM",
        "source_refs": ["MLPredictionTool:AAPL:xgboost_signal:2026-05-13T15:45:00Z"]
      }
    ],
    "bearish_arguments": [
      {
        ...
        "strength": "MEDIUM",
        "source_refs": ["FundamentalsTool:AAPL:current_pe:2026-05-13"]
      }
    ],
...
2. API Layer validate `AdvisoryDecisionRequest` và user constraints.
3. API Layer kiểm tra tool availability cho mode tương ứng.
4. CrewAI khởi tạo Manager Agent và specialist agents với tools đã gắn trong Agent/Task definitions.
5. Manager Agent xác định decision mode và delegate task.
6. Specialist agents query upstream read-only tools trong phạm vi tool được gắn.
7. Specialist agents phân tích tool results và trả structured outputs.
8. Source Quality Checker đánh giá freshness, relevance và completeness của tool results.
9. Manager Agent tổng hợp specialist outputs thành draft decision.
10. Deterministic Conflict Resolution và Confidence Aggregation tính confidence cuối.
11. P2 optional: nếu feature flag bật và có conflict/high-risk/high-confidence, Bullish/Bearish Critic phản biện draft decision.
12. P1 optional: nếu feature flag bật, Compliance Reviewer kiểm tra boundary tư vấn và disclaimer.
13. Rule-Based Validator kiểm tra JSON schema, disclaimer và financial constraints.
14. Human Review Gate được bật nếu thỏa điều kiện rủi ro.
15. Nếu pass, trả final response kèm audit log, `source_refs` và tool result hashes.
16. Nếu fail, Manager Agent revise tối đa 3 lần.
17. Nếu vẫn fail, trả error response có lý do rõ ràng.
```
...
|---|---|---|
| `check_tool_availability` | API Layer | Required tools ready/unavailable |
| `check_source_quality` | Source Quality Checker | Freshness/relevance/completeness score |
| `query_and_summarize_market` | Market Data Agent | Market signal summary |
| `query_and_summarize_sentiment` | Sentiment Agent | Sentiment narrative + drivers |
| `query_and_summarize_valuation` | Valuation Agent | Valuation label + risks |
| `query_and_summarize_risk` | Risk Agent | Asset/portfolio risk summary |
...
| Valuation tool result unavailable | Skip Valuation Agent |
| Critic Agent timeout | Continue without debate, add limitation |
| Compliance Reviewer fail | Reject or revise output |
| Market tool unavailable hoặc required result missing | Reject request |
| Risk tool/result unavailable in portfolio mode | Reject or switch to conservative mode |
...
## 7. Advisory Request và Agent Tool Access Contract

### 7.1 AdvisoryDecisionRequest

`AdvisoryDecisionRequest` là input contract của endpoint. Nó không chứa market/ML/sentiment/valuation/risk context đầy đủ. Các context đó được agents query từ upstream layers qua read-only tools được gắn khi định nghĩa `Agent(...)` hoặc `Task(...)`.

```json
{
  "request_id": "req_20260513_001",
  "timestamp": "2026-05-13T15:45:00Z",
  "as_of_timestamp": "2026-05-13T15:45:00Z",
...
### 7.2 Tool access không nằm trong request

Tool scope là cấu hình của agent/task, không phải field do client truyền. Điều này tránh việc client tự cấp quyền cho agent và bám sát CrewAI pattern hiện hành.

```python
market_data_agent = Agent(
    role="Market Data Analyst",
    goal="Query and summarize grounded market features and ML prediction signals.",
    tools=[market_feature_tool, ml_prediction_tool],
    llm=deepseek_llm,
    verbose=True,
)
...
sentiment_agent = Agent(
    ...
    goal="Query trusted sentiment snapshots and explain news drivers.",
...
valuation_agent = Agent(
    ...
    goal="Assess valuation from upstream fundamentals and benchmarks.",
    tools=[fundamentals_tool, sector_benchmark_tool],
    llm=deepseek_llm,
    verbose=True,
)
...
risk_agent = Agent(
    role="Risk Analyst",
    goal="Evaluate asset-level and portfolio-level risks.",
    tools=[risk_feature_tool, portfolio_tool],
    llm=deepseek_llm,
    verbose=True,
)
```

Task-level tools có thể override hoặc thu hẹp default tools của agent cho một task cụ thể:

```python
market_task = Task(
    description="Query market features for {symbols} as of {as_of_timestamp}. Return JSON only.",
...
### 7.3 Upstream tool result contract

Mọi tool result phải là structured JSON, có timestamp, source refs và trạng thái rõ ràng. Agents chỉ được reasoning trên tool results này và request fields.
...
### 7.4 Required vs optional tool behavior

| Tool result | Required? | Missing behavior |
|---|---|---|
| Market features | Required | Reject request if unavailable or too stale |
| ML predictions | Optional | Mark ML signal `UNAVAILABLE`, reduce confidence |
| Sentiment snapshot | Optional | Sentiment Agent returns `SKIPPED` or low-confidence neutral |
| Valuation snapshot | Optional | Valuation Agent returns `SKIPPED` |
| Risk snapshot | Required for portfolio mode | Reject or switch to conservative mode |
| Portfolio snapshot | Required for portfolio recommendation | Reject if holdings/constraints cannot be loaded |

### 7.5 Auditability of retrieved tool results

Final output không cần echo toàn bộ tool results, nhưng phải có đủ audit metadata để tái lập:
...
```json
{
  "retrieved_tool_audit": {
    ...
    "tool_result_bundle_hash": "sha256:..."
  }
}
```
...
Không dùng recommendation mạnh hơn `BUY` trong P0/P1. Các label như `STRONG_BUY`, `OUTPERFORM`, `ACCUMULATE` hoặc biến thể tương đương không thuộc output contract; nếu cần diễn giải mức độ tích cực, ghi trong rationale nhưng vẫn giữ enum chính là `BUY`.

`confidence` trong final output là confidence cuối cùng sau khi áp dụng risk cap, source-quality cap và conflict downgrade. Các giá trị trung gian phải nằm trong `confidence_breakdown`, tối thiểu gồm:
...
```json
{
  ...
  "portfolio_allocation": [
    ...
    },
    {
      ...
      "rationale": "Stable risk profile and positive valuation tool result."
    },
    {
      "symbol": "CASH",
      "weight_pct": 30,
...
```json
{
  ...
  "status": "ERROR",
  "error_code": "MARKET_TOOL_RESULT_UNAVAILABLE",
  "message": "Market feature tool result is required for investment advisory decisions.",
  "recoverable": true,
  "missing_tool_results": ["MarketFeatureTool.AAPL.latest_price"]
}
```
...
RAG trong hệ thống này không nhằm cho LLM tự tìm kiếm internet tùy ý. RAG và upstream data access được expose dưới dạng read-only tools cho các agents được cấp quyền. Tool result được đưa vào reasoning như structured evidence có timestamp và source refs.

### 9.2 Nguồn grounding

| Source | Tool | Agent sử dụng |
|---|---|---|
| PostgreSQL `real_time_prices` | `MarketFeatureTool` | Market Data Agent |
| PostgreSQL `technical_signals` | `MarketFeatureTool` | Market Data Agent |
| MongoDB `news_documents` | `NewsSentimentTool` | Sentiment Agent |
| MongoDB `news_sentiment` | `NewsSentimentTool` | Sentiment Agent |
| Hive `stock_features` | `MarketFeatureTool`, `FundamentalsTool`, `RiskFeatureTool` | Market/Valuation/Risk Agent |
| Hive `sector_benchmarks` | `SectorBenchmarkTool` | Valuation Agent |
| User Profile DB | `PortfolioTool` hoặc request `user_context` | Manager/Risk Agent |

### 9.3 Retrieval rules

- Retrieval phải xảy ra trước khi agent reasoning.
- Retrieved facts phải có timestamp.
- Retrieved facts phải được trả về dưới dạng structured tool results.
...
Debate/Critic Stage là P2 research extension, không phải dependency của P0. P0 vẫn phải có conflict resolution và confidence aggregation deterministic để xử lý tín hiệu mâu thuẫn trước khi trả final output.
...
- Không che giấu limitation về data freshness, missing tool results hoặc model uncertainty.
...
- Input request và tool result bundle phải hash để kiểm tra thay đổi.
...
3. Tạo `AdvisoryDecisionRequest` cho timestamp đó.
4. Mock hoặc replay upstream tool results chỉ từ dữ liệu có sẵn tại timestamp.
5. Agent sinh recommendation.
6. Simulate outcome theo horizon đã chọn.
7. Tính return/risk metrics.
8. So sánh với baseline.
9. Phân tích lỗi: false BUY, false SELL, missed opportunity, overconfidence.
```
...
1. Agent Layer nhận request tối thiểu và query upstream data/ML layers qua controlled tools.
...
| FR-18 | Agent reasoning phải grounded trong structured tool results và retrieved facts | High | Tất cả agents |
...
| FR-22 | Agent phải trả limitation khi thiếu tool result/metric, không được bịa metric | High | Tất cả agents |
...
| NFR-17 | Graceful degradation khi optional tool results thiếu | Sentiment/Valuation optional | Manager Agent |
...
| NFR-21 | Confidence/recommendation phải reproducible khi request và tool results giống nhau | Best effort với low temperature | Manager + Validator |
...
|---|---|---|
| Market Data Agent thiếu `latest_price` | `MarketFeatureTool.AAPL.latest_price = null` | `ERROR: MARKET_PRICE_UNAVAILABLE` |
| Market Data Agent thiếu ML result | Không có `MLPredictionTool` result | Output vẫn success, `ml_signal=UNAVAILABLE` |
| Sentiment Agent không có news | Empty `NewsSentimentTool` result | `status=SKIPPED` hoặc `NEUTRAL_WITH_LOW_CONFIDENCE` |
| Valuation Agent thiếu fundamentals | Không có `FundamentalsTool` result | `status=SKIPPED` |
| Risk Agent single symbol | Một symbol, không portfolio | Trả asset-level risk |
| Risk Agent portfolio mode thiếu risk result | Không có `RiskFeatureTool.portfolio` hoặc `PortfolioTool` result | Reject hoặc conservative fallback |
...
1. **Happy path:** Đầy đủ market, ML, sentiment, valuation, risk tool results → final decision valid.
2. **Missing valuation:** Không có valuation tool result → Valuation Agent skipped, final response vẫn hợp lệ.
...
9. **Audit log:** Final output có `agent_run_id`, `input_request_hash`, `tool_result_bundle_hash`, `validator_version`.

### 17.3 Hallucination tests

| Scenario | Expected behavior |
|---|---|
| Prompt hỏi P/E nhưng tool result không có P/E | Trả `valuation_metric_unavailable` |
...
- Test serialization/deserialization của `AdvisoryDecisionRequest` và tool result schemas.
...
market_data_agent = Agent(
    role="Market Data Analyst",
    goal="Query and summarize grounded market features and ML prediction signals.",
    backstory="You only use assigned upstream tools and never invent missing metrics.",
    llm=deepseek_llm,
    tools=[market_feature_tool, ml_prediction_tool],
    verbose=True,
)
...
market_task = Task(
    description=(
        ...
        "Return JSON only. Use UNAVAILABLE for missing optional tool results."
    ),
    expected_output="Market signal summary JSON",
    agent=market_data_agent,
)
...
sentiment_task = Task(
    description=(
        "Query NewsSentimentTool and analyze sanitized news snippets. "
...
valuation_task = Task(
    description=(
        "Query fundamentals and sector benchmark tools. If unavailable, return SKIPPED with reason."
    ),
...
risk_task = Task(
    description=(
        "Query RiskFeatureTool and PortfolioTool, then combine with user_context. "
        "Return asset-level or portfolio-level risk summary."
    ),
...
result = crew.kickoff(inputs={
    "request": advisory_decision_request
})
...
Market, ML, sentiment, valuation và risk data không nằm trong request. Các specialist agents query upstream layers qua tools đã được gắn trong Agent/Task definitions.
...
| 503 | `UPSTREAM_TOOL_RESULT_UNAVAILABLE` | Không lấy được tool result bắt buộc |
...
- Nên lưu `AdvisoryDecisionRequest`, tool result hashes, specialist outputs và final output theo `request_id` để phục vụ debugging và demo.
